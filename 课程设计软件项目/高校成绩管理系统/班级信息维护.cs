...
namespace 高校成绩管理系统
{
    public partial class 班级信息维护 : Form
    {
        连接数据库 con = new 连接数据库();
        ...
        protected void SetBind()
        {
            try
            {
                sql = "select wjt_班级编号24 as 班级编号,wjt_班级名称24 as 班级名称,wjt_专业名称24 as 专业名称 from wangjt_班级24,wangjt_专业24 where wangjt_班级24.wjt_专业编号24=wangjt_专业24.wjt_专业编号24";
        ...
        private void 班级信息维护_Load(object sender, EventArgs e)
        {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            try
            {
                sql = "insert into wangjt_班级24 values('" + textBox1.Text + "','" + textBox2.Text + "','" + comboBox1.Text + "')";
                con.OperateDate(sql);
                SetBind();
            }
            catch
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定删除该信息吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                try
                {
                    sql = "delete from wangjt_班级24 where wjt_班级编号24 = '" + dataGView.CurrentRow.Cells[0].Value.ToString() + "'";
                    con.OperateDate(sql);
                    SetBind();
                }
                catch
